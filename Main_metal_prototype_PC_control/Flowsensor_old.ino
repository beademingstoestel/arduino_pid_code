...
bool FLOW_SENSOR_INIT()
{
  ...
  if (returnCode == 0) 
  {
    ...
    delay(1000); // wait 1 second for the sensor to startup
    float currentVal;
    float sum = 0;
    Serial.print("Calibrating flow sensor in 2.5s");
    //LPfiter_init();
    for(int i=0;i<50;i++)
    {
      FLOW_SENSOR_getDP(&currentVal);
      sum+=currentVal;
      delay(50);
    }    
    calibration_offset =sum/50.0;
...
//----------------------------------------------------------------------------------------------------------------
bool FLOW_SENSOR_getDP(float* value)
{
  if (IS_FLOW_SENSOR_INITIALIZED)
  {
    ...
    if (ret == 0) 
    {    
      DP = (sdp.getDifferentialPressure()-calibration_offset);
    } 
    *value = DP;
    return true;
  }
...
//----------------------------------------------------------------------------------------------------------------
float FLOW_SENSOR_convertDP(float value)
{
...
{
  float DP;
  if (FLOW_SENSOR_getDP(&DP))
  {
    *value = FLOW_SENSOR_convertDP(DP);
    return true;
  }
  return false;
}
